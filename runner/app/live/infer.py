...
# Import new architecture components from same directory
...
async def main(
    *,
    ...
    # Create ProcessManager (subprocess management)
    process_manager = ProcessManager(pipeline, params or {})

    # Create session (orchestrator) - only if we have streaming params
    session = None
    if stream_protocol and subscribe_url and publish_url:
        ...
        # Create StreamSession orchestrator
    ...
    api = None
    try:
        # Start ProcessManager (always needed for HTTP API)
        with log_timing("starting ProcessManager"):
            await process_manager.start()

        # Create StreamSession orchestrator if configured for streaming
        if session:
            # Set the ProcessManager instance (don't create a new subprocess)
            session.process_manager = process_manager

            with log_timing("starting StreamSession"):
        ...
        if api:
            try:
                ...
                logging.error(f"Error cleaning up API: {e}")

        try:
            await process_manager.stop()
        except Exception as e:
            logging.error(f"Error stopping process manager: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--events-url",
        type=str,
        help="URL to publish events via Trickle protocol for pipeline status and logs.",
    )
    parser.add_argument(
